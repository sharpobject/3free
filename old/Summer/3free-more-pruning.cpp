...
{
    outputfile = fopen("output-more-pruning.txt", "wt");
...
inline void reset(unsigned char* cantUse)
{
    lastLevel=((N-1)/8);
    goal=setSize[N-1]+1;
    start=GetTickCount();
...
inline void excludeBit(int whichByte,int whichBit,unsigned char* cantUse)
{
    ...
    else    //Shift to the left instead.
    {
        ...
        int bitShift=howFarToShift&7,byteShift=howFarToShift>>3;
        for(int i=0;(i+byteShift)!=whichByte;i++)
        {
            cantUse[i]|=(used[i+byteShift]<<bitShift);
            cantUse[i]|=(used[i+byteShift+1]>>(8-bitShift));
        }
    }
}

inline void copyAToB(unsigned char* ac,unsigned char* bc)
{
...
inline void orAToB(unsigned char* ac,unsigned char* bc)
{
    unsigned int* a=(unsigned int*)ac,*b=(unsigned int*)bc;
    for(int i=0;i<MAXINTS;i++)
        b[i]|=a[i];
    ...
    unsigned char* cantUse)
{
    ...
    for(int i=0;i<nchoices;i++)
    {
        ...
        for(int j=1;j<8;j++)
            if(setSize[N-(level+1)*8+j]<goal-NChosen-ones[subseq&(0xff<<j)])
                continue;
